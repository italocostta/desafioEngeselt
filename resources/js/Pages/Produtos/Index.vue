<template>
  <div class="p-8 bg-accent min-h-screen">
    ...
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-chocolate">Gerenciar Produtos</h1>
      <Link :href="`/confeitarias/${confeitariaId}/produto/create`" class="bg-primary hover:bg-pink-400 text-white font-semibold px-6 py-2 rounded-lg shadow">
        Novo Produto
      </Link>
    </div>
        ...
        <tbody class="bg-white divide-y divide-secondary">
          <tr v-for="produto in produtos" :key="produto.id" class="hover:bg-secondary transition">
            ...
            <td class="px-6 py-4 flex justify-center space-x-2">
              ...
              <button @click="destroy(produto.id)" class="bg-red-400 hover:bg-red-500 text-white px-4 py-2 rounded-lg text-sm">
                Excluir
              </button>
            </td>
          </tr>
